...
def help():
    ...
    print("-list : id or link of youtube list")
    print("-control :  should be already downloaded videos files controlled ?")
    print("-debug : if you want see more than errors")
    print("-output : name of output folder")
    ...
    print("\033[35m" + "----------------------------------------------------------" + "\033[0m")
def error(mesasge,end):
    mesasge
    print("\033[35m" + mesasge + "\033[0m")
    if end:
        sys.exit()
def config():
    # argumenty nastaveni
    parser = argparse.ArgumentParser()
    parser.add_argument("-help", help="extended help", action="store_true")
    parser.add_argument("-list", help="id or link of youtube list")
    parser.add_argument("-control", help="should be already downloaded videos files controlled ?")
    parser.add_argument("-debug", help="if you want see more than errors")
    ...
    if args.control == str(0) or args.control == str(False):
        CONTROL = False
    if args.control == str(1) or args.control == str(True):
        CONTROL = True

    if args.debug == str(0) or args.debug == str(False):
        DEBUG = False
    if args.debug == str(1) or args.debug == str(True):
        DEBUG = True

    if args.list is not None:
        PLAYLIST_ID = args.list

    if args.output is not None:
        MUSIC_FOLDER = args.output

    print("List: ", str(PLAYLIST_ID))
